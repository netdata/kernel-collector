...
static int remove_kprobe_events(const char *val)
{
	int fd, ret, flags;
...
static __u32 choose_kernel_version(__u32 current) 
{
    ...
    __u32 ret;
    if (!parse) {
        ret = (!debian)?KERNEL_VERSION(v_kernel, v_major, v_minor):KERNEL_VERSION(v_kernel, v_major, v_minor) + 20;
...
static int load_and_attach(const char *event, struct bpf_insn *prog, int size, int pid)
{
	...
	if (is_kprobe || is_kretprobe) {
		...
		strcat(buf,(!buf_name[0])?event:buf_name);
		strcat(buf, "/id");
	} else if (is_tracepoint) {
		event += 11;
...
/*
int unload_bpf_file(const char *path)
{
	int fd, i, ret, maps_shndx = -1, strtabidx = -1;
	Elf *elf;
	GElf_Ehdr ehdr;
	...
	if (data_maps) {
		...
		if (load_maps(map_data, nr_maps, NULL))
			goto done;
		map_data_count = nr_maps;
	...
	for (i = 1; i < ehdr.e_shnum; i++) {
		...
		if (memcmp(shname, "kprobe/", 7) == 0 ||
		    ...
		    memcmp(shname, "sk_msg", 6) == 0) {
	                ...
	                bool is_kprobe = strncmp(shname, "kprobe/", 7) == 0;
                        if (!memcmp(shname, "kprobe/", 7) == 0 ||
                        memcmp(shname, "kretprobe/", 10) == 0 )
                        {
			   // snprintf(buf, sizeof(buf), "echo -':%c_netdata_%s_%d' >> /sys/kernel/debug/tracing/kprobe_events",
			   // 	is_kprobe ? 'p' : 'r', find, getpid());
			    snprintf(buf, sizeof(buf), "-:kprobes/%c_netdata_%s_%d",
			    	is_kprobe ? 'p' : 'r', find, getpid());
                            fprintf(stderr,"%s\n",buf);
                            ret = remove_kprobe_events(buf);
			    //ret = system(buf);
			    if (ret != 0)
				goto done;
                        }
