...
#define CHECK_ERR(action, err, out) \
    do {                            \
	err = action;               \
	if (err)                    \
	    goto out;               \
    ...
    __u32* off)
{
	...
	if (!gelf_getsym(symbols, si, &sym))
	    continue;
	if (GELF_ST_BIND(sym.st_info) != STB_GLOBAL || GELF_ST_TYPE(sym.st_info) != STT_OBJECT)
	    continue;
    ...
    const char* pin_root_path)
{
    ...
    if (var_extra->linkage != BTF_VAR_GLOBAL_ALLOCATED && var_extra->linkage != BTF_VAR_STATIC) {
...
static bool bpf_object__shndx_is_data(const struct bpf_object* obj,
    int shndx)
{
    return shndx == obj->efile.data_shndx || shndx == obj->efile.bss_shndx || shndx == obj->efile.rodata_shndx;
}

static bool bpf_object__shndx_is_maps(const struct bpf_object* obj,
    int shndx)
{
    return shndx == obj->efile.maps_shndx || shndx == obj->efile.btf_maps_shndx;
}
    ...
    const GElf_Sym* sym, const GElf_Rel* rel)
{
	...
	for (map_idx = 0; map_idx < nr_maps; map_idx++) {
	    ...
	    if (map->libbpf_type != type || map->sec_idx != sym->st_shndx || map->sec_offset != sym->st_value)
		continue;
...
static int bpf_map_find_btf_info(struct bpf_object* obj, struct bpf_map* map)
{
    ...
    map->btf_value_type_id = bpf_map__is_internal(map) ? ret : value_type_id;
    return 0;
}
...
static bool map_is_reuse_compat(const struct bpf_map* map, int map_fd)
{
    ...
    return (map_info.type == map->def.type && map_info.key_size == map->def.key_size && map_info.value_size == map->def.value_size && map_info.max_entries == map->def.max_entries && map_info.map_flags == map->def.map_flags);
}
...
bpf_object__populate_internal_map(struct bpf_object* obj, struct bpf_map* map)
{
    ...
    data = map->libbpf_type == LIBBPF_MAP_DATA ? obj->sections.data : obj->sections.rodata;
...
bpf_object__create_maps(struct bpf_object* obj)
{
	...
	if (bpf_map_type__is_map_in_map(def->type) && map->inner_map_fd >= 0)
	...
	*pfd = bpf_create_map_xattr(&create_attr);
	if (*pfd < 0 && (create_attr.btf_key_type_id || create_attr.btf_value_type_id)) {
	    err = -errno;
	    cp = libbpf_strerror_r(err, errmsg, sizeof(errmsg));
...
recur:
	...
	return local_len == 0 || targ_len == 0 || (local_len == targ_len && strncmp(local_name, targ_name, local_len) == 0);
    }
    case BTF_KIND_INT:
    ...
    __u32* val, bool* validate)
{
	...
	*val = btf_is_enum(mt) || (btf_int_encoding(mt) & BTF_INT_SIGNED);
	if (validate)
    ...
    struct hashmap* cand_cache)
{
    ...
    if (j == 0 && !prog->obj->relaxed_core_relocs && relo->kind != BPF_FIELD_EXISTS) {
...
bpf_program__get_prog_info_linear(int fd, __u64 arrays)
{
	...
	if (info_len < desc->array_offset + sizeof(__u32) || info_len < desc->count_offset + sizeof(__u32) || (desc->size_offset > 0 && info_len < desc->size_offset))
	    include_array = false;
